...
<div class="max-w-4xl mx-auto">
    <h1 class="text-2xl font-bold mb-6" style="color:var(--ink)">
    ...
    <form method="POST" action="<?= url($isEdit ? '/admin/mail-accounts/' . $account['id'] . '/save' : '/admin/mail-accounts/save') ?>" class="space-y-6">
        ...
        <div class="rounded-lg shadow p-6" style="background:var(--surface)">
            <h2 class="text-lg font-semibold mb-4" style="color:var(--ink)">
                <span class="inline-flex items-center">
                    <svg class="w-5 h-5 mr-2" style="color:var(--dim)" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="name" class="block text-sm font-medium mb-1" style="color:var(--ink-soft)">Nom du compte *</label>
                ...
                <div>
                    <label for="imap_server" class="block text-sm font-medium mb-1" style="color:var(--ink-soft)">Serveur IMAP *</label>
                ...
                <div>
                    <label for="imap_port" class="block text-sm font-medium mb-1" style="color:var(--ink-soft)">Port *</label>
                ...
                <div>
                    <label for="imap_security" class="block text-sm font-medium mb-1" style="color:var(--ink-soft)">Sécurité *</label>
                ...
                <div>
                    <label for="username" class="block text-sm font-medium mb-1" style="color:var(--ink-soft)">Adresse email *</label>
                ...
                <div>
                    ...
                    <input type="password" id="password" name="password" <?= $isEdit ? '' : 'required' ?>
                           placeholder="<?= $isEdit ? '********' : '' ?>"
                           class="form-input">
                    <p class="mt-1 text-xs" style="color:var(--dim)">Pour Gmail, utilisez un mot de passe d'application</p>
                </div>
                ...
                <div class="md:col-span-2">
                    <label class="flex items-center">
                        ...
                        <span class="ml-2 text-sm" style="color:var(--ink-soft)">Compte actif</span>
                    </label>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-lg shadow p-6" style="background:var(--surface)">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="folder" class="block text-sm font-medium mb-1" style="color:var(--ink-soft)">Dossier IMAP à surveiller</label>
                    <input type="text" id="folder" name="folder"
                           value="<?= htmlspecialchars($account['folder'] ?? 'INBOX') ?>"
                           placeholder="INBOX"
                           class="form-input">
                    <p class="mt-1 text-xs" style="color:var(--dim)">Ex: INBOX, Factures, Documents</p>
                </div>

                <div>
                    <label for="processed_folder" class="block text-sm font-medium mb-1" style="color:var(--ink-soft)">Dossier après traitement</label>
                    <input type="text" id="processed_folder" name="processed_folder"
                           ...
                           class="form-input">
                    <p class="mt-1 text-xs" style="color:var(--dim)">Laisser vide pour ne pas déplacer</p>
                </div>

                <div>
                    <label for="check_interval" class="block text-sm font-medium mb-1" style="color:var(--ink-soft)">Intervalle de vérification</label>
        ...
        <div class="rounded-lg shadow p-6" style="background:var(--surface)">
            ...
            <p class="text-sm mb-4" style="color:var(--dim)">Filtrer les emails à traiter. Laisser vide pour tout traiter.</p>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="filter_from" class="block text-sm font-medium mb-1" style="color:var(--ink-soft)">Filtrer par expéditeur</label>
                ...
                <div>
                    <label for="filter_subject" class="block text-sm font-medium mb-1" style="color:var(--ink-soft)">Filtrer par sujet</label>
        ...
        <div class="rounded-lg shadow p-6" style="background:var(--surface)">
            ...
            <p class="text-sm mb-4" style="color:var(--dim)">Ces valeurs seront appliquées aux documents importés depuis ce compte.</p>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    <label for="default_document_type_id" class="block text-sm font-medium mb-1" style="color:var(--ink-soft)">Type de document</label>
                ...
                <div>
                    <label for="default_correspondent_id" class="block text-sm font-medium mb-1" style="color:var(--ink-soft)">Correspondant</label>
                ...
                <div>
                    <label for="default_folder_id" class="block text-sm font-medium mb-1" style="color:var(--ink-soft)">Dossier de destination</label>
...
<div class="max-w-4xl mx-auto mt-8">
    <div class="rounded-lg shadow p-6" style="background:var(--surface)">
        ...
        <div id="ingestion-logs" class="text-sm" style="color:var(--dim)">
            Chargement...
        </div>
    </div>
</div>
            ...
            if (data.success && data.logs.length > 0) {
                ...
                data.logs.forEach(log => {
                    ...
                    html += `<tr class="border-b" style="border-color:var(--border)">
                        ...
                        <td class="py-2" style="${statusClass}">${log.status}</td>
                    </tr>`;
                });
                html += '</tbody></table>';
                document.getElementById('ingestion-logs').innerHTML = html;
            } else {
                document.getElementById('ingestion-logs').innerHTML = '<p style="color:var(--dim)">Aucun historique d\'ingestion</p>';
            }
        })
        .catch(() => {
            document.getElementById('ingestion-logs').innerHTML = '<p style="color:var(--dim)">Impossible de charger l\'historique</p>';
        });
});
</script>
<?php endif; ?>
