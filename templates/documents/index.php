...
<div class="flex min-h-screen bg-gray-100" style="margin: -1rem -1rem;">
    ...
    <aside class="w-64 bg-white border-r border-gray-200 overflow-y-auto">
        ...
        <!-- Dossiers filesystem - Afficher tous les dossiers réels du filesystem -->
        ...
        <nav class="p-2">
            <a href="<?= url('/documents') ?>" 
               class="flex items-center px-3 py-2 text-sm rounded-lg hover:bg-gray-100 mb-1
                ...
                <span class="flex-1">Tous les dossiers</span>
            </a>
            ...
            <a href="<?= url('/documents?folder=' . urlencode($folder['id'])) ?>" 
               class="flex items-center px-3 py-2 text-sm rounded-lg hover:bg-gray-100
                      <?= ($folderId == $folder['id']) ? 'bg-blue-50 text-blue-700' : 'text-gray-700' ?>">
                <span class="mr-2"><?= str_repeat('&nbsp;&nbsp;', $folder['depth']) ?><?= $folder['depth'] > 0 ? '└─' : '' ?></span>
                <span class="flex-1"><?= htmlspecialchars($folder['name']) ?></span>
                <span class="text-xs text-gray-400"><?= $folder['file_count'] ?? 0 ?></span>
            </a>
            <?php endforeach; ?>
        </nav>
        <?php endif; ?>
